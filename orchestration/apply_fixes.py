...
def apply_diff(task: dict, repo_path: str, dry_run: bool = False) -> dict:
    ...
    # ── Attempt 1: git apply ──────────────────────────────────────────────────
    # If the patch creates new files (--- /dev/null), remove existing untracked file
    # so git apply won't fail with 'already exists in working directory'
...
def run_pytest(repo_path: str) -> dict:
    ...
    try:
        ...
        # Exit code 4 = no tests collected; exit code 5 = no tests found — both fine.
        # If the failure is purely an ImportError during collection (missing deps in
        # the target repo), skip rather than roll back — the diff itself is good.
...
def main() -> None:
    ...
    for task in tasks:
        ...
        if not apply_result["applied"]:
            # git apply --check failed or dry-run
        ...
        if not pytest_result["pytest_passed"]:
            # Pytest failed — rollback the applied patch
        ...
        commit_result = git_commit(repo_path, task_id, feature_request)
