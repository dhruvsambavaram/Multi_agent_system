...
def _save_task_for_dashboard(task: dict) -> None:
    """Persist the final task JSON to dashboard/tasks/{task_id}.json."""
    ...
    try:
        ...
        try:
            home_dir = os.path.join(os.path.expanduser("~"), ".ai_dev_team", "tasks")
            os.makedirs(home_dir, exist_ok=True)
...
def run_pipeline(
    ...
    if task["status"] == "blocked":
        ...
        return _finish(task, agent_timings=agent_timings)
...
def _finish(task: dict, agent_timings: list = None) -> dict:
